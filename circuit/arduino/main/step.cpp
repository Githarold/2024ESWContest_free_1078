...
int IN1[2] = {6, 3};       // 모터 A와 B의 입력 1
int IN2[2] = {7, 4};       // 모터 A와 B의 입력 2
const int ENB[2] = {5, 5}; // 모터 A와 B의 활성화 핀
int IN3[2] = {3, 6};       // 모터 A와 B의 입력 3
int IN4[2] = {4, 7};       // 모터 A와 B의 입력 4
...
void disk_rotate(int disk_step) {
    if (disk_step == 0) {
        ...
        digitalWrite(ENA[0], LOW); // Disable motor A
        ...
        digitalWrite(ENA[1], LOW); // Disable motor A
        digitalWrite(ENB[1], LOW); // 반시계방향 비활성화
    }
    else {
